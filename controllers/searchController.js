...
exports.getDomainData = async (domain) => {
  ...
  async function getKeywords() {
    const URL = `https://www.spyfu.com/NsaApi/Serp/GetAllOrganicKeywordLists?query=${domain}&countryCode=US`;
    try {
      const res = await axios.get(URL);
      console.log(res.status, res.statusText);
      const result = res.data;
  ...
  async function getTopPages() {
    const URL = `https://www.spyfu.com/NsaApi/Serp/GetTopPages?domain=${domain}&pageSize=5&sortOrder=descending&sortBy=estMonthlySeoClicks&filter=&startingRow=1&isOverview=true&countryCode=US`;
    try {
      const res = await axios.get(URL);
      console.log(res.status, res.statusText);
      const result = res.data;
